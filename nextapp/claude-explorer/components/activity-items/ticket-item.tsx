...
export function TicketItem({ raw, onStartChat }: TicketItemProps) {
  const priority = raw.priority !== undefined ? PRIORITY_ICON[raw.priority] : null;
      ...
      <div className="flex items-start gap-2.5 px-3 py-2.5">
        ...
        <div className="mt-1 shrink-0">
          <span
            ...
            style={{ borderColor: raw.statusColor, backgroundColor: `${raw.statusColor}20` }}
            title={raw.status}
          />
        </div>
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            {priority && (
              <span className={cn("shrink-0 text-[10px] font-bold", priority.className)}>
                {priority.icon}
              </span>
            )}
          </div>
