...
// All project-detail sections extracted from app/project/[slug]/page.tsx
// These are now rendered inside the left sidebar when on a project page.
...
export function ProjectSkills({ slug }: { slug: string }) {
  ...
  return (
    <div className="mb-1 rounded border">
      <button
                    ...
                    {mcpTransport === "stdio" ? (
                      <>
                        <input
                          ...
                          className="w-20 rounded border bg-background px-1.5 py-0.5 text-[11px]"
                        />
                        <input
                          placeholder="Args"
                          value={mcpArgs}
  ...
  return (
    <div className="mb-1">
      <button
        ...
        className="mb-1 flex items-center gap-1 text-xs font-medium text-muted-foreground hover:text-foreground w-full px-1"
      >
      ...
      {showFiles && isLoading && (
        <div className="py-1 text-[10px] text-muted-foreground animate-pulse px-1">
          loading...
        </div>
      )}
      {showFiles && entries && (
        <>
          <div className="mb-1 flex items-center gap-2 text-[10px] px-1">
            <button
...
export function ProjectTmux({ slug }: { slug: string }) {
...
export function TmuxLauncher({ slug }: { slug: string }) {
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <label className="flex items-center gap-1 text-xs">
              <input
                ...
                className="h-3 w-3"
              />
              <span className="text-muted-foreground">Skip perms</span>
            </label>
            <select
              value={model}
          ...
          {showTips && (
            <div className="rounded border bg-muted/20 px-3 py-2 text-[11px] text-muted-foreground">
              <ul className="flex flex-col gap-1">
                {noTmux ? (
                  <>
                    <li>Multiple instances run as background processes</li>
                    <li>Pair with nohup if needed for SSH disconnect</li>
                  </>
                ) : (
                  <>
...
export function ProjectCrons({ slug }: { slug: string }) {
  const queryClient = useQueryClient();
...
export function ProjectWebhooks({ slug }: { slug: string }) {
  const queryClient = useQueryClient();
  const [showSection, setShowSection] = useState(false);
  const [showForm, setShowForm] = useState(false);
  const { data: allWebhooks } = useQuery({
            ...
            <div className="flex flex-col gap-1.5">
              {webhooks.map((wh) => (
                ...
                >
                  ...
                  />
                  <span className="shrink-0 text-xs font-medium truncate">
                    {wh.name}
                  </span>
...
export function ProjectClaudeMd({ slug }: { slug: string }) {
  ...
  return (
    <div className="mb-1">
      ...
      {show && (
        <pre className="max-h-48 overflow-auto rounded border p-2 text-[11px] text-muted-foreground">
          {config.claudeMd}
        </pre>
      )}
    </div>
...
export function ProjectStatsHeader({ slug }: { slug: string }) {
  const [show, setShow] = useState(false);
  ...
  return (
    <div className="mb-1">
      ...
      {show && (
        <div className="flex flex-wrap gap-3 rounded border px-2 py-1.5">
          {stats.map((s) => (
