...
interface CronsContentProps {
  projectSlug?: string;
}

export function CronsContent({ projectSlug: scopedSlug }: CronsContentProps) {
  ...
  const [projectSlug, setProjectSlug] = useState(scopedSlug ?? "");
  ...
  // Scoped filtering
  const displayCrons = useMemo(
    () =>
      scopedSlug ? crons?.filter((c) => c.projectSlug === scopedSlug) : crons,
    [crons, scopedSlug]
  );

  const scopedCronIds = useMemo(() => {
    if (!scopedSlug || !crons) return null;
    return new Set(
      crons.filter((c) => c.projectSlug === scopedSlug).map((c) => c.id)
    );
  }, [crons, scopedSlug]);
  ...
  const displayEvents = useMemo(
    () =>
      scopedCronIds
        ? events?.filter((ev) => scopedCronIds.has(ev.cronId))
        : events,
    [events, scopedCronIds]
  );
  ...
  return (
    <div className="p-4">
      ...
      ) : !displayCrons || displayCrons.length === 0 ? (
