 ...
 * DockerTerminal
 *
 * Full interactive browser terminal backed by a PTY running inside the
 * container. Connects to the standalone WebSocket server on port 3001
 * (terminal-ws-server.ts) using xterm.js.
 *
 * Usage:
 *   <DockerTerminal />                          — opens shell in HOME
 *   <DockerTerminal cwd="/home/bun/projects" /> — opens shell in cwd
 */
...
/** Derive the WebSocket URL for the terminal server (port 3001). */
function getWsUrl(cwd?: string): string {
  const protocol = window.location.protocol === "https:" ? "wss:" : "ws:";
  const host = window.location.hostname;
  const url = new URL(`${protocol}//${host}:3001`);
...
export function DockerTerminal({ cwd, className }: DockerTerminalProps) {
  ...
  useEffect(() => {
    ...
    ws.onopen = () => {
      // Send initial size in case the server uses query params as a hint only
      ws.send(
